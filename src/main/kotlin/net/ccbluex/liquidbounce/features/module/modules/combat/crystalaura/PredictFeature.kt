 ...
 *
 * Bug fix vs. original: the original used fixed tick counts for
 * [placeTicks] / [destroyTicks] / [basePlaceTicks] regardless of the
 * player's actual round-trip ping to the server. On high-ping servers
 * this made the prediction use stale snapshots, causing damage estimates
 * to be off by `ping / 50` ticks.
 *
 * We now optionally add `player.ping / 50` extra ticks to each prediction
 * when [autoPingCompensation] is enabled. This is on by default but can
 * be disabled for low-ping servers where the extra ticks would over-
 * extrapolate.
 */
abstract class PredictFeature(name: String) : ToggleableConfigurable(ModuleCrystalAura, name, true) {

    /**
     * Automatically adds `player.ping / 50` extra ticks to each prediction
     * to compensate for network latency. Disable on LAN or low-ping
     * servers where the extra ticks would over-extrapolate.
     */
    private val autoPingCompensation by boolean("AutoPingCompensation", true)
    ...
    /**
     * Effective number of ticks to predict for the place action, including
     * any ping compensation. Computed on every read so changes to
     * [autoPingCompensation] take effect immediately.
     */
    val effectivePlaceTicks: Int
        get() = placeTicks + pingCompensationTicks()

    val effectiveDestroyTicks: Int
        get() = destroyTicks + pingCompensationTicks()

    val effectiveBasePlaceTicks: Int
        get() = basePlaceTicks + pingCompensationTicks()

    private fun pingCompensationTicks(): Int {
        if (!autoPingCompensation) return 0
        // Convert round-trip ping (ms) to ticks. Cap at 6 ticks (300ms)
        // so extreme ping spikes don't blow up the simulation.
        return (player.ping / 50).coerceIn(0, 6)
    }
    ...
    fun willBeBlocked(box: Box, target: PlayerEntity?, basePlace: Boolean): Boolean {
        ...
        val ticks = if (basePlace) effectiveBasePlaceTicks else effectivePlaceTicks
        val simulation = getSnapshotPos(target, ticks)
    ...
    fun getDamage(
        ...
        // Apply ping compensation on top of the caller-provided tick count.
        val effectiveTicks = ticks + pingCompensationTicks()
        val simulated = getSnapshotPos(player, effectiveTicks)
