...
object SubmoduleBasePlace : ToggleableConfigurable(ModuleCrystalAura, "BasePlace", true) {
    ...
    private val placer = tree(BlockPlacer(
        ...
        slotFinder = { _ ->
            // Bedrock is not obtainable in survival, so don't waste a slot search on it.
            // Prefer offhand over hotbar so the main hand stays free for swords / crystals.
            Slots.OffhandWithHotbar.findSlot(Items.OBSIDIAN)
        }
    ))
    ...
    fun shouldBasePlaceRun(): Boolean {
        ...
        // Don't start a new attempt if we are already busy with one.
        // currentTarget being non-null counts as one in-flight attempt,
        // which prevents packet spam when many positions become valid
        // at the same time.
     ...
     *
     * Bug fix: the original returned `true` (safe) when SimulateMovement was
     * ENABLED, which made the check a no-op exactly when the user wanted
     * it active. The correct semantics is: skip when disabled, run when
     * enabled.
     *
     * Bug fix 2: the original code expanded X/Z by `errorStep` but Y by
     * `errorOffset` (the running accumulator). That made the Y expansion
     * grow much faster than X/Z and never reset between candidates.
     * Use `error + i * errorStep` for the per-tick expansion on every axis.
     */
    private fun playerWillNotRunIn(pos: BlockPos): Boolean {
        ...
        val baseError = SimulateMovement.error.toDouble()
        ...
        return snapShots.none {
            ...
            val expansion = baseError + idx * errorStep
            idx++
            val result = posBB.intersects(
                simulatedPos.x - expansion,
                simulatedPos.y - if (errorDown) expansion else 0.0,
                simulatedPos.z - expansion,
                simulatedPos.x + 1.0 + expansion,
                simulatedPos.y + 1.8 + expansion,
                simulatedPos.z + 1.0 + expansion
            )
     ...
     *
     * Bug fix: the original used `floor(bb.maxX)` for `yB`, which is a typo -
     * it should be `floor(bb.maxY)`. Using maxX meant the second wall layer
     * and the ceiling were computed from an X coordinate, making the entire
     * trap-detection logic wrong.
     */
