...
object CrystalAuraDamageOptions : Configurable("Damage") {
    ...
    /**
     * Bug fix: the original hardcoded a blast-resistance threshold of 9f
     * (roughly Netherrack-tier) to decide which blocks to exclude from the
     * exposure calculation. On servers with custom blocks this gave wrong
     * damage estimates. We now expose this threshold as a configurable
     * value so users can tune it for their server.
     *
     * Blocks with a blast resistance BELOW this value are treated as
     * "will be blown away" and excluded from the exposure calculation.
     * Vanilla reference values:
     *   - 0.0  = ignore no blocks (treat everything as solid)
     *   - 9.0  = Netherrack-tier (vanilla default, original hardcode)
     *   - 30.0 = Stone-tier (only obsidian / bedrock count as cover)
     *   - 1.5  = dirt/wood-tier
     */
    private val terrainBlastResistance by float("TerrainBlastResistance", 9.0f, 0.0f..1200.0f)
    ...
    private fun LivingEntity.getDamage(
        ...
        val calc: (DamageConstellation) -> DamageProvider = {
            ...
            checkedEntity.getDamage(
                ...
                requestingSubmodule,
                crystal,
                if (excludeNotBlastResistant) terrainBlastResistance else null,
    ...
    private enum class CheckedEntity {
        ...
        SELF {
            override fun getDamage(
                ...
                // Use the raw tick count; ping compensation is applied
                // centrally inside PredictFeature.getDamage().
