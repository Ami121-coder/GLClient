...
object SubmoduleCrystalDestroyer : ToggleableConfigurable(ModuleCrystalAura, "Destroy", true) {
    ...
    /**
     * Bug fix: the original declared this as a fixed `arrayOf(...)` which
     * made it impossible for other modules to register their own
     * post-attack handlers without editing the source. We use a mutable
     * list and expose [addHandler] / [removeHandler] so other modules
     * (or future submodules) can hook in.
     */
    private val _postAttackHandlers = mutableListOf<CrystalPostAttackTracker>(
        CrystalAuraSpeedDebugger,
        SubmoduleSetDead.CrystalTracker
    )

    val postAttackHandlers: List<CrystalPostAttackTracker>
        get() = _postAttackHandlers.toList()

    fun addHandler(handler: CrystalPostAttackTracker) {
        if (handler !in _postAttackHandlers) {
            _postAttackHandlers.add(handler)
        }
    }

    fun removeHandler(handler: CrystalPostAttackTracker) {
        _postAttackHandlers.remove(handler)
    }
